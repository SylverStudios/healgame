...
**How to use:** Check out branch **`poc/spell-card-upgrades`**. Select everything
...
**Feel / fantasy input (chips, not a relic catalog):**
...
Start from branch **`poc/spell-card-upgrades`** (planning docs already committed).
...
- Free level unlocks for spells + major CDs (handoff §3).
- Spend **upgrade points** on **fixed authored 3-way chip choices** per slot
  (NO RNG in PoC). Each upgradable spell has **6 chips**: slot-1 trio
  (synergy / teach) + slot-2 trio (wilder playstyle forks).
- Chip fantasy draws from `docs/relic-revamp-brainstorm.md` (Z/S/R “change the
  decision”) but implements only existing `CombatMods` hooks.
- Resolve into **`CombatMods`** — engine unchanged.

Bible (read fully before coding):
`docs/spell-cards-poc-handoff.md`

Also read:
- `docs/relic-revamp-brainstorm.md` §§1–4, §6 (feel only — do not implement
  engine-heavy wild relics like Martyr’s Mirror)
- `CLAUDE.md`, `AGENTS.md`
- `game/src/data/loadout.ts`, `game/src/save/save.ts`
- `game/src/data/radial/resolve.ts` (pattern to mirror)
...
4. Spend flow: pick spell with free slot → **exactly the 3 authored chips for
   that slot** → pick 1 → chip sticks → combat uses baked mods (Arming Mend /
   Battle Mend / Graven / Steady etc. per handoff §8).
...
| 2 | subagent | All 24 chips (fixed slot sets), offersForNextSlot (no RNG), resolve, purchase validation, synergy + missing/full HP tests |
...
Pinned contracts + chip tables: handoff §§7–8. **Do not invent RNG pools.
Do not invent alternate unlock levels. Do not reintroduce RelicScene in cards.**
...
- Every subagent prompt includes: handoff path; CREATE / MAY EDIT / DO NOT TOUCH;
  copied API signatures from handoff §7; locked micro-decisions; DoD = verify:fast;
...
- Chip ids and slot trios must match handoff §8 exactly (retune magnitudes OK).

================================================================================
SCOPE / NON-GOALS
================================================================================

Reject: deleting lattice/radial; shipping relic catalog/art; RNG chip pools;
Big Heal / specializes / oaths; CD chip pools; respec; new engine mechanics
(Martyr’s Mirror, Silent Choir, Ash Confession, etc.); party-stat chips; soft
save migration.
...
3. Short note in PR: any reversible defaults you picked; remind reviewers chip
   offers are fixed (no RNG) and relic-revamp is the feel reference.
````
