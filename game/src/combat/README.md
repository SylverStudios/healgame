...
- **Tank block (v1 M3)**: `CombatEngineOptions.blockThresholdN` enables
  deterministic every-N mitigation for the tank only. The engine maintains an
  integer `blockDamageCarry` (starts 0). On each hit to the tank after armor:
  `carry += postArmorDamage; blocked = floor(carry / N); carry %= N;
  finalDamage = max(0, postArmorDamage - blocked)`. The armor step floors at 1
  (existing `damageAfterArmor` rule) — the block step does **not** re-apply
